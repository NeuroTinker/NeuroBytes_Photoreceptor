...
extern volatile uint16_t active_output_pins[11] = {PIN_DARK_EX,PIN_LIGHT_EX,0,0,0,0,0,0,0,0,0};
...
extern uint8_t  downstream_port[3] = {0,0,0}; // 0=DARK, 1=LIGHT, 2=BOTH
extern uint8_t  downstream_port_i = 0;
...
void writeDownstream(void)
{
    ...
    //write_buffer.downstream_port = 3;
    switch (write_buffer.downstream_port){
        case 1:
            if (value != 0){
...
void writeAll(void)
{
    ...
    active_output_ports[0] = PORT_DARK_OUT;
    active_output_pins[0] = PIN_DARK_EX;
    active_output_ports[1] = PORT_LIGHT_OUT;
    active_output_pins[1] = PIN_LIGHT_EX;
    for (i=0;i<2;i++){
